...
const sendTelegramNotification = async (chatId, type, details = {}, teamId) => {
  ...
  // ensure we include both an HTML anchor and a plain URL fallback
  // safeUrl: use projectUrl if provided otherwise create a best-effort fallback
...
const HandoverPopup = ({ teamId, handoverId, taskId, onClose, membersDetails = [], currentUserUid }) => {
  ...
  const resolveName = (uidOrId) => {
    ...
    // Prefer membersDetails lookup by uid fields
    ...
    // fallback: try current auth user
    ...
    // lastly, return raw uid for debugging (helps avoid 'Unknown')
    return uid;
  };
  ...
  const generateTaskUrl = () => {
    // Prefer a stable unique identifier: taskNumber -> taskData.id -> taskId (prop)
    const identifier = taskData?.taskNumber || taskData?.id || taskId || '';
    // Fallback host (when window.location may be undefined in some environments)
    ...
    // If we don't have an identifier, return the team page so the link still goes somewhere useful
    if (!identifier) return `${origin}/team/${teamId}`;
    return `${origin}/team/${teamId}?taskId=${encodeURIComponent(identifier)}`;
  };
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-[95vw] h-[92vh] flex flex-col overflow-hidden animate-fade-in-up">
        ...
        <div className="flex-1 overflow-hidden flex flex-col lg:flex-row bg-slate-50">
              ...
              <div className="space-y-6">
                ...
                {(taskData.status === 'Revision' || taskData.revisionFeedback) && (
                  <div className="bg-red-50 p-5 rounded-lg border border-red-200">
                    ...
                    <div className="mt-2 text-[10px] text-red-400">Requested by {resolveName(taskData.revisionFeedback?.requestedBy)} on {taskData.revisionFeedback?.requestedAt ? new Date(taskData.revisionFeedback.requestedAt).toLocaleString() : '—'}</div>
                  </div>
                )}
                ...
                {(((taskData.images && taskData.images.length > 0) || (taskData.files && taskData.files.length > 0) || (taskData.links && taskData.links.length > 0))) && (
                  <div className="bg-white rounded-lg border border-slate-200 shadow-sm p-6">
                    ...
                    <div className="flex gap-3 flex-wrap">
                      {taskData.images?.map((img, i) => (<div key={i} className="relative group"><img src={img} className="h-24 w-24 object-cover rounded-lg border shadow-sm cursor-zoom-in hover:scale-105 transition" onClick={() => setPreviewImage(img)} alt={`img-${i}`} /></div>))}
                ...
                {taskData.submission && (
                  <div className="bg-purple-50 p-6 rounded-lg border border-purple-100 shadow-sm">
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <span className="text-[10px] text-purple-400">{taskData.submission.submittedAt ? new Date(taskData.submission.submittedAt).toLocaleString() : ''}</span>
                    </div>
                    <p className="text-sm text-slate-700 mb-4 whitespace-pre-wrap">{taskData.submission.note}</p>
                    <div className="flex gap-2 overflow-x-auto pb-2">
                      {taskData.submission.images?.map((img, i) => <img key={i} src={img} className="h-16 w-16 object-cover rounded border border-purple-200 cursor-zoom-in hover:opacity-80" onClick={() => setPreviewImage(img)} alt={`simg-${i}`} />)}
          ...
          <div className="w-full lg:w-[320px] border-l border-slate-200 bg-white flex flex-col h-full z-20">
            ...
            {/* COMMENT ALERT BANNER */}
            {commentAlert && (
              <div
                className="p-3 bg-yellow-50 border-b border-yellow-100 text-sm text-yellow-800 cursor-pointer"
                onClick={() => {
              ...
              {comments.map((msg) => (
                <div key={msg.id} className="group">
                  <div className="flex justify-between items-baseline mb-1 px-1">
                    <span className="font-bold text-slate-700 text-xs">{msg.userName || (msg.userId ? resolveName(msg.userId) : 'Unknown')}</span>
                    <span className="text-[10px] text-slate-400">{msg.createdAt?.toDate ? msg.createdAt.toDate().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : ''}</span>
                  </div>
            ...
            <form onSubmit={async (e) => {
              ...
              const userName = auth.currentUser?.displayName || resolveName(currentUserUid);
              const commentPayload = { text: newComment, userId: auth.currentUser.uid, userName, createdAt: serverTimestamp() };
              try {
                await addDoc(collection(db, 'teams', teamId, 'handovers', handoverId, 'comments'), commentPayload);
                setNewComment('');
                // Send Telegram notification about comment to team/channel
                const projectUrl = generateTaskUrl();
                sendTelegramNotification(telegramChatId, 'comment', { userName: commentPayload.userName, projectName: projectData?.title, taskName: taskData?.title, notes: newComment, projectUrl }, teamId);
            ...
            <div className="flex-1 overflow-y-auto space-y-4">
              ...
              <div>
                ...
                <div className="flex gap-2 mt-2 flex-wrap">
                  {submissionImages.map((img, i) => <img key={i} src={img} className="h-10 w-10 object-cover rounded border" alt={`subimg-${i}`} />)}
