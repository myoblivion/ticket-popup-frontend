...
// --- HandoversSection (main component) ---
const HandoversSection = ({ teamId, membersDetails = [], isTeamCreator, currentUserUid, selectedProjectId, myTaskStatusFilter }) => {
  ...
  }, [teamId, selectedProjectId, currentUserUid, myTaskStatusFilter]);
  ...
  return (
    <div className="flex flex-col h-full bg-white rounded-lg">
      <div className="p-4 border-b border-gray-200 flex flex-wrap gap-3 items-center bg-gray-50/50 rounded-t-lg">
        ...
        <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} className="border border-gray-300 rounded-md px-3 py-1.5 text-sm bg-white">
        ...
        <select value={filterPriority} onChange={e => setFilterPriority(e.target.value)} className="border border-gray-300 rounded-md px-3 py-1.5 text-sm bg-white">
        ...
        <select value={filterAssignee} onChange={e => setFilterAssignee(e.target.value)} className="border border-gray-300 rounded-md px-3 py-1.5 text-sm bg-white">
          <option value="">All Assignees</option>
          {membersDetails.map(m => <option key={m.uid} value={m.uid}>{m.displayName || m.email}</option>)}
        </select>
        ...
        <div className="flex items-center gap-2 border-l pl-3 ml-2 border-gray-300">
          {selectedProjectId && selectedProjectId !== 'my_tasks' && isTeamCreator && (
            <button onClick={() => setIsManageMembersModalOpen(true)} className="flex items-center gap-2 bg-white border border-gray-300 text-gray-700 px-3 py-1.5 rounded-md text-xs font-bold hover:bg-gray-50 transition shadow-sm" title="Manage Project Members">
              <UserGroupIcon /><span className="hidden sm:inline">Members</span>
            </button>
          )}
      ...
      <div className="flex-1 overflow-auto p-0">
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200 text-sm">
            {loading ? (
              <tr><td colSpan="10" className="py-10 text-center"><div className="animate-spin h-5 w-5 border-2 border-blue-500 border-t-transparent rounded-full mx-auto"></div></td></tr>
            ) : filteredTasks.length === 0 ? (
              <tr><td colSpan="10" className="text-center py-10 text-gray-400 italic">No tasks found. Select a project and click New Task.</td></tr>
            ) : (
              filteredTasks.map((task, idx) => {
                ...
                const displayId = task.taskNumber ? `#${task.taskNumber}` : `-${idx + 1}`;
                ...
                return (
                  ...
                  >
                    <td className="px-6 py-4 text-gray-500 text-xs font-mono font-bold">{displayId}</td>

                    <td className="px-6 py-4">
                      <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-gray-100 text-gray-800">{task.category || 'General'}</span>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <span className="text-xs text-gray-500 line-clamp-2" title={task.description}>{task.description || '-'}</span>
                    </td>

                    <td className="px-6 py-4">
                      <span className={`px-2 py-1 rounded border text-xs font-bold uppercase ${getStatusColor(task.status)}`}>{task.status || 'Open'}</span>
                    </td>

                    <td className="px-6 py-4">
                      <span className={`text-xs font-bold ${task.priority === 'High' ? 'text-red-600' : task.priority === 'Low' ? 'text-gray-500' : 'text-blue-600'}`}>{task.priority || 'Medium'}</span>
                    </td>

                    <td className="px-6 py-4">
                      {Array.isArray(task.assignedTo) && task.assignedTo.length > 0 ? (
                        <div className="flex flex-col text-xs text-gray-700">
                          {task.assignedTo.map(uid => <span key={uid} className="truncate max-w-[150px]" title={resolveName(uid)}>{resolveName(uid)}</span>)}
                        </div>
                      ) : (
                    ...
                    {/* Creator Column */}
                    <td className="px-6 py-4">
                      <span className="text-xs text-gray-600 truncate max-w-[100px] block" title={resolveName(task.createdBy || task.projectCreator)}>
                        {resolveName(task.createdBy || task.projectCreator)}
                      </span>
                    </td>

                    <td className="px-6 py-4">
                      <div className="flex flex-col text-xs"><span className="text-gray-600">{formatDate(task.createdAt)}</span></div>
                    </td>
